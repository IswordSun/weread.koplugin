...
function PrivateBadge:paintTo(bb, x, y)
    ...
    local body_height = math.max(4, math.floor(self.size * 0.25))
    local body_x = x + math.floor((self.size - body_width) / 2)
    local body_y = center_y
    local shackle_radius = math.max(2, math.floor(body_width / 2))
    local shackle_x = center_x
    local shackle_y = body_y
    bb:paintRect(body_x, body_y, body_width, body_height, Blitbuffer.COLOR_BLACK)
    for row = 0, shackle_radius do
        local half_width = math.floor(math.sqrt(shackle_radius * shackle_radius - row * row))
        bb:paintRect(shackle_x - half_width, shackle_y - row,
            2 * half_width + 1, 1, Blitbuffer.COLOR_BLACK)
    end
    local shackle_inner_radius = math.max(0, shackle_radius - 2)
    for row = 0, shackle_inner_radius do
        local half_width = math.floor(math.sqrt(
            shackle_inner_radius * shackle_inner_radius - row * row
        ))
        bb:paintRect(shackle_x - half_width, shackle_y - row - 1,
            2 * half_width + 1, 1, Blitbuffer.COLOR_WHITE)
    end
end
